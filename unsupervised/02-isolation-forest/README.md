...
2. **Build Isolation Tree** — recursively partition: pick a random feature, pick a random split value between the feature's min and max, separate data into left (< split) and right (≥ split) until each point is isolated or max depth $\lceil\log_2(\text{max\_samples})\rceil$ is reached.
